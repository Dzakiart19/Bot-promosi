...
async function runCommentSession() {
  ...
  for (const viewkey of targets) {
    ...
    try {
      ...
      // 401 = cookies expired — hentikan siklus, beri tahu operator
      ...
      // 422 captcha-score — session terflag, cooldown panjang
      if (/captcha-score/i.test(err.message)) {
        log("WARN", "[PH] CAPTCHA score terlalu rendah — cooldown 45 menit");
