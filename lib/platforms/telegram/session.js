 ...
 * Dua fungsi utama:
 *
 * 1. createMessageListener(client, botEntity)
 *    → Pasang SATU event handler permanen di client.
 *    → Kembalikan fungsi nextMsg(timeoutMs) yang baca dari buffer tanpa race condition.
 *    → Semua pesan dari bot target masuk ke buffer, tidak ada yang terlewat meski
 *      ada gap antar sesi.
 *
 * 2. runSession(client, botEntity, nextMsg)
 *    → Satu siklus: tunggu match → kirim promo langsung → delay 5s → /next.
 *    → "Jangan terlalu cepat" dan pesan lain yang bukan match-signal diabaikan
 *      otomatis dalam loop tunggu match.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
function createMessageListener(client, botEntity) {
  ...
  function nextMsg(timeoutMs) {
    return new Promise((resolve, reject) => {
      // Kalau buffer sudah ada isi, langsung ambil
 ...
 *   1. Tunggu "Pasangan telah ditemukan" (abaikan pesan lain)
 *   2. Kirim promo langsung
 *   3. Delay DELAY_NEXT_MS (default 5 detik)
 *   4. Kirim /next
 *
 * @param {import("telegram").TelegramClient} client
 * @param {*}        botEntity  hasil client.getEntity()
 * @param {Function} nextMsg    dari createMessageListener
...
async function runSession(client, botEntity, nextMsg) {
  ...
  while (true) {
    ...
    // Abaikan pesan bukan match-signal ("Jangan terlalu cepat", dll)
  ...
  // ── Kirim promo LANGSUNG (tanpa delay) ───────────────────────────────────
