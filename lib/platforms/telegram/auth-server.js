...
function _createServer() {
  ...
  // ── Logout: cabut sesi dari Telegram + hapus dari DB & file ─────────────────
  app.post("/api/logout", async (req, res) => {
    const { clearSession } = require("./persistence");
    try {
      // Coba panggil auth.LogOut() ke Telegram supaya sesi benar-benar dicabut
      const { readSession } = require("./persistence");
      ...
      if (sessionStr) {
        try {
          const logoutClient = new TelegramClient(
            ...
            API_ID, API_HASH,
            { connectionRetries: 2, retryDelay: 1000,
              baseLogger: (() => {
                const { Logger } = require("telegram/extensions/Logger");
                const l = new Logger();
                l.levels = []; l._logLevel = null;
                l.canSend = () => false; l._log = () => {};
                return l;
              })() }
          );
          await logoutClient.connect();
          await logoutClient.invoke(new Api.auth.LogOut());
          log("SUCCESS", "[AUTH] Sesi dicabut dari Telegram ✓");
        } catch (e) {
          // Lanjut hapus lokal walau RPC gagal (misal session sudah invalid)
  ...
  // ── Proxy /api/telegram-auth/:action (agar dashboard di port lain bisa trigger OTP) ──
