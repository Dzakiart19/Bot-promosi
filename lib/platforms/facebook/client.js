...
function parseVideoIdsFromHtml(html) {
  const ids = [...html.matchAll(/"video_id":"(\d{10,20})"/g)].map(m => m[1]);
  return [...new Set(ids)];
}

/**
 * Ambil feedback_id asli dari halaman video individu.
 * Format feedback_id Facebook terbaru: feedback:POST_ID_REPLY_ID (underscore)
 * Hanya bisa didapat dengan mengambil halaman video spesifik.
 *
...
async function fetchVideoFeedbackId(cookieStr, videoId) {
  try {
    ...
    // Cari feedback_id langsung dari HTML video page
    // Format: ZmVlZGJhY2s6POST_ID_REPLY_ID (base64 encoded)
    const fbIds = [...html.matchAll(/feedback_id[":\s,]+"([A-Za-z0-9+/=]{20,})"/g)]
      .map(m => m[1]);

    if (!fbIds.length) return null;

    // Pilih yang paling sederhana (tanpa trailing underscore content jika ada)
    // Coba ambil yang paling pendek (root feedback, bukan nested reply)
    // Dari feedback_id manapun (termasuk format reply POST_ID_COMMENT_ID),
    // ekstrak POST_ID (bagian sebelum underscore pertama) lalu compute root.
    const anyDecoded = Buffer.from(fbIds[0], "base64").toString("utf8");
    const postId     = anyDecoded.replace(/^feedback:/, "").split("_")[0];
    // Root post feedback_id = base64("feedback:POST_ID") — tidak ada underscore
    const feedbackId = Buffer.from(`feedback:${postId}`).toString("base64");

    log("INFO", `[FB] video ${videoId} → post ${postId} (rootFeedback: ${feedbackId})`);
...
async function postComment(cookieStr, tokens, docId, feedbackId, text) {
  ...
  const body = new URLSearchParams({
    ...
    variables:  JSON.stringify({
      input: {
        ...
        message:           { text },
        feedback_source:   "PERMALINK",
        idempotence_token: crypto.randomUUID(),
        session_id:        crypto.randomUUID(),
        is_tracking_checked: false,
        attribution_id_v2: "",
      },
    }),
    doc_id: String(docId),
  ...
  // Cek error — hanya throw jika CRITICAL (bukan WARNING).
  // Facebook sering mengembalikan warning-level errors bersamaan dengan data sukses.
  if (!data?.data && data?.errors?.length) {
    const criticals = data.errors.filter(e => (e.severity ?? "CRITICAL") !== "WARNING");
    if (criticals.length) {
      const msg = criticals.map(e => e.message).join("; ");
      throw new Error(`GraphQL error: ${msg}`);
    }
    // Semua WARNING tanpa data → lempar semua
    if (!data?.data) {
  ...
  const disabled = data?.data?.comment_create?.feedback
    ?.comments_disabled_notice_renderer;
  if (disabled) {
    return { commentId: null, success: false, error: "comments_disabled" };
  }

  // Ambil ID komentar baru
  const commentId = data?.data?.comment_create?.comment?.id ?? null;
