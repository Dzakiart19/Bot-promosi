...
/**
 * Mode COMMENT (keyword-filtered):
 * ambil trending → filter keyword → comment pada post relevan yang belum dikomentari.
 *
 * Search API GETTR tidak berfungsi server-side, jadi sumber post adalah trending
 * yang sudah difilter keyword di fetchTrendingPosts() — mirip REPLY mode X Bot.
 */
async function runCommentSession(session) {
  stats.status = "searching";
  log("BOT", "[GETTR] Ambil trending posts (filter keyword)...");
  ...
  if (!candidates || candidates.length === 0) {
    stats.totalNoMatch++;
    pushEvent("search", "Tidak ada trending post yang cocok keyword — skip siklus ini");
    return "no-target";
  }

  // Filter: belum dikomentari (uid sudah difilter di fetchTrendingPosts via session.handle)
  const targets = candidates.filter(
    (p) => !repliedStore.has(p.id)
  );
  ...
  for (const target of toComment) {
    ...
    try {
      ...
      // Log strategi yang dipakai — thread-reply = muncul di thread asli ✓
      //                              profile-reply = hanya di tab Replies profil ⚠
      const strategyLabel = result.strategy === "thread-reply"
        ? "✓ thread-reply (muncul di thread)"
        : "⚠ profile-reply (hanya di Replies profil)";
      log("SUCCESS", `[GETTR] Komentar terkirim ke ${target.id} → id ${result.id || "?"} [${strategyLabel}]`);
      pushEvent("reply", `[GETTR][${result.strategy}] → https://gettr.com/post/${target.id}`);
      commented++;
    } catch (err) {
      stats.totalErrors++;
      stats.lastErrorAt  = Date.now();
