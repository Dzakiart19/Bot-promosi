...
/**
 * Post komentar/reply ke sebuah post GETTR.
 *
 * Recon Juli 2026 (live API test):
 *   - POST /api/u/post/{postId}/comment → ERR "parent postId != null" untuk semua root post.
 *   - POST /api/u/post/{c37pId}/comment → rc OK tapi mengembalikan ID yang SAMA dengan
 *     parent (data parent dikembalikan, tidak ada comment baru dibuat — ghost comment).
 *   - POST /api/u/post + { pid: postId } di body → rc OK, ID baru unik (p426...),
 *     reply tampil di bawah post target ✓  ← SATU-SATUNYA YANG BENAR.
 *
 * @returns {{ id, strategy }} ID post-reply baru dan strategi yang dipakai
 */
async function postComment(session, postId, text) {
  ...
  return { id: newId, strategy: "direct-reply" };
}
...
// fetchFirstComment tidak lagi diekspos — endpoint /post/{c37p}/comment terbukti
// mengembalikan data parent (ghost comment), bukan comment baru. Lihat postComment().
