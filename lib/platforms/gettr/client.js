...
/**
 * Post komentar (true reply) ke post GETTR.
 *
 * Endpoint yang benar: POST /api/u/post/{postId}/comment
 *   - pid di-encode di URL path, BUKAN di body
 *   - body: { content: { txt, uid (numeric), _t: "cmt" } }
 *   - Menghasilkan ID format "c37p..." (true comment), bukan "p426..." (post)
 *   - Komentar muncul di Replies tab akun dan di thread original post ✓
 *
 * Endpoint lama (/api/u/post + pid di body) menghasilkan post biasa dengan
 * pid tersimpan tapi tidak ter-render sebagai reply di Replies tab.
 *
 * @returns {{ id }} ID komentar baru
 */
async function postComment(session, postId, text) {
  // pid ada di URL path — endpoint yang benar untuk true comment
  const url = `${cfg.API_BASE}/u/post/${postId}/comment`;
