...
/**
 * Post komentar (reply) ke post GETTR.
 *
 * Endpoint: POST /api/u/post
 *   - pid disertakan di body content — GETTR menyimpan relasi parent dengan benar
 *   - Komentar memiliki field pid tersimpan → muncul di thread original post ✓
 *   - ID yang dihasilkan format "p426..." (sama seperti post biasa) — GETTR tidak
 *     mengekspos ID format "c37p..." via endpoint ini, tapi comment tetap muncul
 *     di Replies tab karena pid field terisi.
 *
 * Catatan: Endpoint /api/u/post/{postId}/comment mengembalikan
 * "parent postId: {id} != null" untuk semua post di trending — blocked oleh GETTR
 * server-side untuk akun ini. Endpoint /u/post + pid di body terbukti bekerja.
 *
...
async function postComment(session, postId, text) {
  ...
  const body = JSON.stringify({
    content: {
      txt: text,
      uid: session.username,   // numeric user ID (harus cocok dengan x-app-auth user)
      _t:  "cmt",
      pid: postId,             // parent post ID — membuat comment muncul di thread
    },
  });
