 ...
 * Session valid jika HTML halaman utama mengandung data user (bukan redirect login).
 */
function verifySession(html) {
  // Halaman logged-in mengandung userId atau userinfo
  const isLoggedIn = /"userId":"(\d+)"/.test(html) ||
                     /"ig_user_id":"(\d+)"/.test(html) ||
                     /class="x.*?logout/.test(html) ||
                     (html.includes("sessionid") && !html.includes("LoginPage"));

  if (!isLoggedIn) {
    // Cek apakah ini redirect ke login
    if (html.includes("ThreadsLoginPage") || html.includes("/login")) {
      throw new Error("Session Threads tidak valid / expired — login ulang dan update THREADS_COOKIES");
    }
    // Kalau tidak jelas, anggap valid (Threads sometimes serves minimal HTML)
  }
 ...
 * @param {string} [htmlCache] — kalau HTML sudah diambil sebelumnya, pakai kembali
 */
...
/** Invalidate LSD cache (kalau 401/403 dari API). */
...
async function graphqlRequest(cookies, lsd, docId, variables) {
  const body = encodeBody({ lsd, doc_id: docId, variables });
