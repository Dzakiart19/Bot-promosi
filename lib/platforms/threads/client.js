...
function extractFbDtsg(html) {
  // Metode 1: DTSGInitData JSON inline
  const dtsgMatch = html.match(/DTSGInitData[^{]*\{[^}]*"token"\s*:\s*"([^"]+)"/);
  if (dtsgMatch) return dtsgMatch[1];

  // Metode 2: "dtsg":{"token":"<value>"}
  const dtsgMatch2 = html.match(/"dtsg"\s*:\s*\{"token"\s*:\s*"([^"]+)"/);
  if (dtsgMatch2) return dtsgMatch2[1];

  // Metode 3: fb_dtsg hidden input
  const inputMatch = html.match(/name="fb_dtsg"\s+value="([^"]+)"/);
  if (inputMatch) return inputMatch[1];

  // Metode 4: "token":"<long token>" pola Meta (token lebih panjang dari LSD)
  const tokenMatch = html.match(/"token"\s*:\s*"([A-Za-z0-9_\-]{30,100})"/);
  if (tokenMatch) return tokenMatch[1];

  return null; // opsional — bisa null, request tetap dicoba
}
...
async function getLsd(cookies, htmlCache) {
  ...
  if (cachedFbDtsg) log("INFO", `[Threads] fb_dtsg: ${cachedFbDtsg.slice(0, 20)}…`);
  return cachedLsd;
}
