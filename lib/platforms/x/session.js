...
/**
 * Pilih hashtag acak dari pool tanpa duplikat, gabung jadi string.
 * @param {number} [count] — override jumlah; default: cfg.HASHTAG_PICK_COUNT
 */
function pickHashtags(count) {
  const pool = [...cfg.HASHTAG_POOL];
  const n    = Math.min(count ?? cfg.HASHTAG_PICK_COUNT, pool.length);
  const picked = [];
  for (let i = 0; i < n; i++) {
...
async function runReplySession(account) {
  ...
  try {
    const replyText = pickRandom(cfg.REPLY_TEXTS) + "\n" + pickHashtags();
...
async function runCommentSession(account) {
  ...
  try {
    ...
    stats.lastErrorAt  = Date.now();
    stats.lastErrorMsg = err.message;
    log("ERROR", `[COMMENT] HomeTimeline gagal: ${err.message}`);
...
async function runPostSession(account) {
  ...
  const text = pickRandom(cfg.POST_TEXTS) + "\n" + pickHashtags(cfg.HASHTAG_PICK_COUNT_POST);
