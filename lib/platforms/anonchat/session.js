...
function runSession(guest) {
  return new Promise(async (resolve) => {
    ...
    // ── update-dialog-id (PROTOKOL BARU — menggantikan partner-found) ────────
    // Setelah recon Juli 2026: server mengirim "update-dialog-id" sebagai sinyal
    // match, bukan lagi "partner-found". dialogId ada di data.dialogId (bukan data._id).
    socket.on("update-dialog-id", (data) => {
      ...
      log("SUCCESS", `✓ MATCH! dialogId=${dialogId}`);
      pushEvent("match", `Dialog assigned: ${String(dialogId).slice(0, 16)}…`);

      setTimeout(() => {
        if (done) return;
        sendGreeting();
        messageSent = true;
        pushEvent("send", `Sapa dikirim ke dialog ${String(dialogId).slice(0, 12)}`);
    ...
    // ── partner-found (PROTOKOL LAMA — fallback kompatibilitas) ──────────────
    socket.on("partner-found", (data) => {
      if (done || matched) return;
      matched = true;
      clearTimeout(matchTimer);

      dialogId = data?._id || data?.dialogId;
      ...
      log("SUCCESS", `✓ MATCH (partner-found)! dialogId=${dialogId} | uid=${uid}`);
