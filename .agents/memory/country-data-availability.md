...
**Why:** country-based filtering/prioritization can only be implemented for OpenTalk and Chatib until other platforms are re-checked (a future protocol change could add the field).

**How to apply:** shared blocklist/priority-list logic lives in `lib/core/country-filter.js` (`isBlockedCountry`, `isPriorityCountry`). Any new country-based feature should hook into the same module and only wire it into OpenTalk/Chatib session handlers unless new recon proves otherwise for other platforms.
